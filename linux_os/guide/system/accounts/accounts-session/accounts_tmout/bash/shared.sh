...
for f in /etc/profile /etc/profile.d/*.sh; do
{{% endif %}}
    # Validamos que el archivo exista para evitar errores con el *.sh
    if [ -f "$f" ] && grep --silent '^[^#].*TMOUT' "$f"; then
        sed -i -E "s/^(.*)TMOUT\s*=\s*(\w|\$)*(.*)$/typeset -xr TMOUT=$var_accounts_tmout\3/g" "$f"
        tmout_found=1
    fi
done

# Si no se encontró en ningún lado, lo crea en profile.d (comportamiento por defecto)
if [ $tmout_found -eq 0 ]; then
        echo -e "\n# Set TMOUT to $var_accounts_tmout per security requirements" >> /etc/profile.d/tmout.sh
        echo "typeset -xr TMOUT=$var_accounts_tmout" >> /etc/profile.d/tmout.sh
fi

# Y ADEMÁS, garantizamos que se escriba en /etc/bashrc si no estaba ahí
if ! grep --silent '^[^#].*TMOUT' /etc/bashrc; then
        echo -e "\n# Set TMOUT to $var_accounts_tmout per security requirements" >> /etc/bashrc
        echo "typeset -xr TMOUT=$var_accounts_tmout" >> /etc/bashrc
fi
